...
import {PDFDict, PDFDocument, PDFName, PDFNumber, PDFRawStream, StandardFonts} from 'pdf-lib';
...
// };
//
// class App extends Component<{}, { file?: File, data: any }> {
//
...
//         this.state = {
//             file: undefined,
//             data: this.flattenObject(baseData, null, {})
//         };
//     }
//
//     flattenObject = (obj: any, keyName: any, baseObject: any) => {
...
//                 let lastIndex = 0
//                 while (true) {
//                     let index = text.substr(lastIndex).indexOf(key)
//                     if (index === -1) {
//                         break
//                     }
//                     found = true
//                     text = this.insertAt(text, value, index, index + key.length)
//                     lastIndex = index + value.length - key.length
//                 }
//             } else {
...
//     }
//
//     decodeStream = (stream: any) => {
//         return codecs.decode(stream.getContents())
//     }
//
//     encodeStream = (baseDict: any, data: any, encoding = 'FlateDecode') => {
...
//         matches.push(textObjectOperator.exec(block));
//         matches.map((e: any) => {
//             let str, found;
//             const textOperator = /\[\(|]TJ|\(|\)([-.0-9]+)/g;
//             if (e && e[1] != null) {
...
//             block = this.insertAt(block, data, begin, begin + match[1].length)
...
//                     const streamSet = doc.getPages().map(page => {
//                         return page.node
//                     }).map(node => [
...
//
//                         let content = this.decodeStream(stream)
//                         let any;
//                         while (true) {
...
//                                         text = text.replace(matchStr, this.state.data[str])
//                                     }
//                                     return text;
//                                 }]
//                             ];
...
//                             doc.getPage(0).drawText('Hello world', {
//                                 x: 0,
//                                 y: 0,
//                                 size: 30
//                             });
